...
|---|---|---|---|---|
| Zone 4 | 0.1825 | 0.1929 | 0.1822 ± 0.0021 | **5.4%** |
| Zone 7 | 0.1476 | 0.1806 | 0.1456 ± 0.0041 | **18.3%** |

A 与 C 都包含双胞胎（分别 0.1825、0.1822），B 不包含（0.1929）。
**差异完全来自「双胞胎在不在训练集」**：去掉它带来的损失，
远超去掉任意一个无关场站的损失。

统计上也站得住：B 的取值超出 C 重复测量 5 至 8 个标准差，不是噪声。

**结论**：按场站划分交叉验证时，共享输入数据的场站会互相泄漏，
使模型性能被高估 5% 至 18%。做多场站实验必须按**组**划分。
...
模型性能分别被高估 **5.4%** 与 **18.3%**。完整设计、量化方法与工具代码见上文的
「数据质量检验」一节（`src/data_qc.py`）。
